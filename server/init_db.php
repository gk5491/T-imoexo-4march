...
try {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $updateStmt = $pdo->prepare("UPDATE admin_users SET password = ?, email = ? WHERE username = ?");
    ...
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_blog_status ON blog_posts(status);");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_blog_created ON blog_posts(created_at);");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_blog_slug ON blog_posts(slug);");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_media_type ON media_library(file_type);");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_comment_status ON blog_comments(status);");
    $pdo->exec("CREATE INDEX IF NOT EXISTS idx_comment_blog_id ON blog_comments(blog_post_id);");
